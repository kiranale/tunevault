/**
 * routes/unusableindexes.js — Unusable Indexes on-demand endpoint.
 *
 * Owns: fetching counts of unusable indexes at three granularities.
 * Does NOT own: health check execution, user auth state, Oracle connection storage.
 *
 * Mounted at: /api (see server.js)
 *
 * POST /api/health-checks/:id/unusable-indexes
 *   Returns per-owner counts of unusable indexes (whole), partitions, and
 *   subpartitions from DBA_INDEXES, DBA_IND_PARTITIONS, DBA_IND_SUBPARTITIONS.
...
const { getDemoUnusableIndexes } = require('../demo-data');
 ...
 * POST /api/health-checks/:id/unusable-indexes
 *
 * Returns per-owner counts of unusable indexes (whole indexes, partitions,
 * subpartitions) from DBA_INDEXES, DBA_IND_PARTITIONS, DBA_IND_SUBPARTITIONS.
 * Available on all Oracle editions — no license gating.
 */
router.post('/health-checks/:id/unusable-indexes', requireAuth, async (req, res) => {
  try {
    const hc = await loadHcAndGuard(req, res);
    if (!hc) return;

    if (hc.is_demo) {
      return res.json(getDemoUnusableIndexes());
    }
    ...
    const result = await oracle.queryUnusableIndexes(connParams);
    res.json(result);
  } catch (err) {
    console.error('[unusableindexes] Error fetching unusable indexes:', err);
    res.status(500).json({ error: 'Failed to fetch unusable indexes' });
  }
});
